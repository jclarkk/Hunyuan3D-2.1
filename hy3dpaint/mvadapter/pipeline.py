...
class MVAdapterPipelineWrapper:
    ...
    def __call__(self,
                 ...
                 prompt: str = "high quality, albedo only, no lighting",
                 negative_prompt: str = "watermark, ugly, deformed, noisy, blurry, low contrast, baked lighting, ambient occlusion, shadows, specular, reflection, gloss, highlight, glow, directional light, rim lighting, shading",
        ...
        # Ensure control_images has enough views for processing
        # Pad if necessary to handle edge cases
        if control_images.shape[0] < num_views:
            pad_count = num_views - control_images.shape[0]
            last_image = control_images[-1:].repeat(pad_count, 1, 1, 1)
            control_images = torch.cat([control_images, last_image], dim=0)
            # Pad pos_images and normal_images for debug output consistency
            if isinstance(pos_images, list):
                pos_images += [pos_images[-1]] * pad_count
            if isinstance(normal_images, list):
                normal_images += [normal_images[-1]] * pad_count

        views_per_batch = 6

        if num_views % 6 != 0:
            # Pad control_images to next multiple of 6
            pad_count = (6 - (num_views % 6))
            num_views_padded = num_views + pad_count
            if control_images.shape[0] < num_views_padded:
                last_image = control_images[-1:].repeat(num_views_padded - control_images.shape[0], 1, 1, 1)
                control_images = torch.cat([control_images, last_image], dim=0)
        else:
            num_views_padded = num_views

        generator = torch.Generator(device=self.device).manual_seed(seed) if seed != -1 else None
        all_images = []

        for batch_start in range(0, num_views_padded, views_per_batch):
            batch_end = batch_start + views_per_batch
            batch_control_images = control_images[batch_start:batch_end]

            # Ensure we have exactly 6 control images for this batch
            if batch_control_images.shape[0] < 6:
                pad_count = 6 - batch_control_images.shape[0]
                last_image = batch_control_images[-1:].repeat(pad_count, 1, 1, 1)
                batch_control_images = torch.cat([batch_control_images, last_image], dim=0)
        ...
        # Ensure we drop any excess images if num_views is not a multiple of batch_size since we might have padded
        all_images = all_images[:num_views]
