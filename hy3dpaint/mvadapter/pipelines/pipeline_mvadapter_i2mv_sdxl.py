    ...
    def combine_metallic_roughness(self, metallic_maps, roughness_maps, output_type="pil"):
        ...
        if output_type == "pil":
            ...
            for i in range(batch_size):
                ...
                mr_images.append(PIL.Image.fromarray(mr_array))
            return mr_images
        else:
            return mr_tensor
        ...
        **kwargs,
        ...
        if output_pbr_maps:
            ...
            if pbr_prompts is None:
                pbr_prompts = {
                    'metallic': 'metallic surface, chrome material, steel, reflective metal, shiny',
                    'roughness': 'rough texture, matte surface, bumpy, coarse material, grainy'
                }

            # Generate metallic map
            metallic_prompt = pbr_prompts.get('metallic', 'metallic chrome steel reflective shiny metal')
            ...
            # Use shallow denoising for metallic
            metallic_latents = final_latents.clone()
            metallic_timesteps = timesteps[-5:]  # Only last few steps
            ...
            metallic_maps = 0.299 * metallic_maps[:, 0:1] + \
                            0.587 * metallic_maps[:, 1:2] + \
                            0.114 * metallic_maps[:, 2:3]
            metallic_maps = torch.sigmoid((metallic_maps - 0.5) * 4)

            # Generate roughness map
            roughness_prompt = pbr_prompts.get('roughness', 'rough matte texture bumpy coarse surface')
            ...
            roughness_maps = torch.sigmoid((roughness_maps - 0.5) * 4)
