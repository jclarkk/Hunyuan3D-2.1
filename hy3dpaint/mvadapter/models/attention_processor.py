...
class LoRAMVAttnProcessor(DecoupledMVRowColSelfAttnProcessor2_0):
    """
    Multi-view aware attention processor with LoRA adaptation.
    Combines view-aware attention patterns with low-rank adaptation.
    """
    ...
    def __init__(
        ...
        # Add LoRA layers for base attention
        self.to_q_lora = LoRALinearLayer(query_dim, inner_dim, rank, lora_alpha)
        self.to_k_lora = LoRALinearLayer(query_dim, inner_dim, rank, lora_alpha)
        self.to_v_lora = LoRALinearLayer(query_dim, inner_dim, rank, lora_alpha)
        self.to_out_lora = LoRALinearLayer(inner_dim, query_dim, rank, lora_alpha)

        # Add LoRA layers for multi-view attention if enabled
        if self.use_mv:
            self.to_q_mv_lora = LoRALinearLayer(query_dim, inner_dim, rank, lora_alpha)
            self.to_k_mv_lora = LoRALinearLayer(query_dim, inner_dim, rank, lora_alpha)
            self.to_v_mv_lora = LoRALinearLayer(query_dim, inner_dim, rank, lora_alpha)
            self.to_out_mv_lora = LoRALinearLayer(inner_dim, query_dim, rank, lora_alpha)

        # Add LoRA layers for reference attention if enabled
        if self.use_ref:
            self.to_q_ref_lora = LoRALinearLayer(query_dim, inner_dim, rank, lora_alpha)
            self.to_k_ref_lora = LoRALinearLayer(query_dim, inner_dim, rank, lora_alpha)
            self.to_v_ref_lora = LoRALinearLayer(query_dim, inner_dim, rank, lora_alpha)
            self.to_out_ref_lora = LoRALinearLayer(inner_dim, query_dim, rank, lora_alpha)
    ...
    def __call__(
        ...
        # Get LoRA scale from cross_attention_kwargs if available
        if "scale" in kwargs:
            self.lora_scale = kwargs["scale"]

        # Use runtime num_views if provided
        num_views = num_views if num_views is not None else self.num_views
        if num_views is None:
            raise ValueError("num_views must be provided at runtime or during initialization.")
        ...
        # Apply base attention with LoRA
        query = attn.to_q(hidden_states) + self.lora_scale * self.to_q_lora(hidden_states)

        if use_mv:
            query_mv = self.to_q_mv(hidden_states) + self.lora_scale * self.to_q_mv_lora(hidden_states)
        if use_ref:
            query_ref = self.to_q_ref(hidden_states) + self.lora_scale * self.to_q_ref_lora(hidden_states)
        ...
        # Multi-view attention with LoRA
        if use_mv:
            key_mv = self.to_k_mv(encoder_hidden_states) + self.lora_scale * self.to_k_mv_lora(encoder_hidden_states)
            value_mv = self.to_v_mv(encoder_hidden_states) + self.lora_scale * self.to_v_mv_lora(encoder_hidden_states)
            ...
            # Apply view-aware attention patterns (row-col decomposition)
            query_mv = rearrange(
                query_mv,
                "(b nv) (ih iw) h c -> b nv ih iw h c",
                nv=num_views,
            ...
            col_views_indices = [0, 2, 4, 5][:min(4, num_views)]
            if len(col_views_indices) > 0:
                query_mv_col = query_mv[:, col_views_indices]
                key_mv_col = key_mv[:, col_views_indices]
                value_mv_col = value_mv[:, col_views_indices]
                query_mv_col = rearrange(
            ...
            hidden_states_mv_out = self.to_out_mv[0](hidden_states_mv)
            hidden_states_mv_out = hidden_states_mv_out + self.lora_scale * self.to_out_mv_lora(hidden_states_mv)
            hidden_states_mv = self.to_out_mv[1](hidden_states_mv_out)

        # Reference attention with LoRA
        if use_ref and ref_hidden_states is not None and self.name in ref_hidden_states:
            reference_hidden_states = ref_hidden_states[self.name]
            key_ref = self.to_k_ref(reference_hidden_states) + self.lora_scale * self.to_k_ref_lora(
                reference_hidden_states)
            value_ref = self.to_v_ref(reference_hidden_states) + self.lora_scale * self.to_v_ref_lora(
                reference_hidden_states)
            ...
            # Apply output projection with LoRA
            hidden_states_ref_out = self.to_out_ref[0](hidden_states_ref)
            hidden_states_ref_out = hidden_states_ref_out + self.lora_scale * self.to_out_ref_lora(hidden_states_ref)
            hidden_states_ref = self.to_out_ref[1](hidden_states_ref_out)

        # Final output projection with LoRA
        hidden_states_out = attn.to_out[0](hidden_states)
        hidden_states_out = hidden_states_out + self.lora_scale * self.to_out_lora(hidden_states)
