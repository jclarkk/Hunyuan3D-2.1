...
from fastmesh.models.modules.transformers import AdaLNSelfAttn
...
class FaceGen(nn.Module):
    def __init__(self, args):
        ...
        self.asloss = AsymmetricLoss(gamma_neg=3, gamma_pos=0, clip=0.05, eps=1e-8, disable_torch_grad_focal_loss=True,
                                     scaler=1e4)
    ...
    def forward(self,
        ...
        with torch.no_grad():
            ...
            xyz_ids = torch.tensor([0, 1, 2]).reshape(1, 1, 3).repeat(input_ids.shape[0], input_ids.shape[1], 1).to(
                input_ids.device)  # Batch, N Vert, 3
        ...
        pad_attention_mask = torch.ones((BATCH, self.cond_length), device=attention_mask.device,
                                        dtype=attention_mask.dtype)
        attention_mask = torch.concatenate((pad_attention_mask, attention_mask), dim=1).float()
        train_mask = attention_mask[:, self.cond_length:].unsqueeze(-1) @ attention_mask[
            :, self.cond_length:].unsqueeze(-2)
