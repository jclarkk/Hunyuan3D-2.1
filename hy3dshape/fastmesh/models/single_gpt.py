...
from fastmesh.models.miche.encode import load_model
from fastmesh.models.models import register
from fastmesh.models.shape_opt import ShapeOPTConfig
from fastmesh.models.vert_refine import VertRefine
...
class SingleGPT(nn.Module):
    ...
    def pad_id_and_attn(self, input_ids, attention_mask, face_ids=None):  # same
        ...
        input_ids[
            attention_mask.bool()] += 3  # 0 - num_tokens to 3 - num_tokens + 3, total: 0 - num_tokens + 3, num: numtokens + 4
        input_ids[
            ~attention_mask.bool()] = self.pad_token_id  # in transformers pad token id is only used for init nn.embedding which we won't use
    ...
    def train_one_step(self, data_dict: dict):
        ...
        pad_attention_mask = torch.ones((attention_mask.shape[0], self.cond_length), device=attention_mask.device,
    ...
    def generate(self, data_dict) -> dict:
        ...
        if self.use_refine:
            ...
            gen_vert = torch.nn.utils.rnn.pad_sequence(gen_vert, batch_first=True, padding_value=-1,
                                                       padding_side='right').to(point_feature.device)
        else:
            refined_vertices = torch.nn.utils.rnn.pad_sequence(gen_vert, batch_first=True, padding_value=-1,
                                                               padding_side='right').to(point_feature.device)
