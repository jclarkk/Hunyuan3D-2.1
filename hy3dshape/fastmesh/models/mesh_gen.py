...
from fastmesh.models.models import register, MODELS
from fastmesh.utils import load_model
...
class MeshGen(nn.Module, PyTorchModelHubMixin):
    def __init__(self, **kwargs):
        ...
        if self.vert_args['vertgen_ckpt'] is not None:
            load_model(torch.load(self.vert_args['vertgen_ckpt'], map_location=torch.device("cpu"), weights_only=False)[
                           "model"], self.vert_gen)
        if self.face_args['vertgen_ckpt'] is not None:
            load_model(torch.load(self.face_args['facegen_ckpt'], map_location=torch.device("cpu"), weights_only=False)[
                           "model"], self.face_gen)
